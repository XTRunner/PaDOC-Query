...
def main():
    pfolderName = "ldaModel_12"

    if not os.path.isdir(pfolderName):
        os.mkdir(pfolderName)

    if os.path.exists(pfolderName + "/trainCleanedText.csv"):
        print("Start Loading Cleaned Data...")
        with open(pfolderName + "/trainCleanedText.csv", 'r') as handle:
        ...
        for each_f in training_files:
            ...
            with open(pfolderName + "/trainCleanedText.csv", 'a', newline='') as whandle:
    ...
    lda_model = lda_train(population, 12)

    for i in range(lda_model.num_topics):
        print(lda_model.print_topics()[i])

    # Save to disk
    temp_file = datapath(os.getcwd() + "\\" + pfolderName + "\\ldaTrainedModel")
